...
function RequestCard({
  ...
  async function handleAccept() {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/request/${req.requestId}/accept`, {
        method: 'POST',
...
export function OpenRequests({ language, onBack }: Props) {
  ...
  async function fetchRequests() {
    ...
    const token = localStorage.getItem('agrochain_token');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/request/open`, {
        headers: { Authorization: `Bearer ${token ?? ''}` },
        signal: AbortSignal.timeout(3000),
      });
